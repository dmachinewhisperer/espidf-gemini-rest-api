...
static esp_err_t http_event_handler(esp_http_client_event_t *evt) {
    ...
    switch(evt->event_id) {
        ...
        case HTTP_EVENT_ON_DATA:
            ...
            output_len += copy_len;
            #if 0
             //Only process response if it is not chunk-encoded 
            if (!esp_http_client_is_chunked_response(evt->client)) {
                ...
                if(cJSON_GetObjectItem(json, "candidates")){
                    ...
                    int copy_len = 0;
                    copy_len = MIN(evt->data_len, MAX_GENERATED_TEXT_LENGTH);
                    if(copy_len){
                        char *gen_text = extract_text_from_json(json);
                        strncpy(((PromptConf *)evt->user_data)->gen_text, gen_text, copy_len + 1);
                        ((PromptConf *)evt->user_data)->gen_text[copy_len] = '\0';
                        free(gen_text);
                    }
                }
                ...
                if(file_obj){
                    ...
                    if(file_uri){
                        if(((PromptConf *)evt->user_data)->file_uri){
                            free(((PromptConf *)evt->user_data)->file_uri);
                        }
                        ((PromptConf *)evt->user_data)->file_uri = strdup(evt->header_value);
                    }
...
/**
 * @todo: the args can be one as conf is assecible via client.user_data
 */
